    ...
    [void] Set() {
        if ($this.Ensure -eq [Ensure]::Present) {
            ...
            if ($this.installNav($this)) {
                ...
                Set-Service -Name "MicrosoftDynamicsNavServer`$$($this.InstanceName)" -Status Running -StartupType Automatic -PassThru
...
<Configuration>
    ...
    <Parameter Id="NavServiceAccount" Value="$($obj.ServiceAccount.UserName)"/>
    <Parameter Id="NavServiceAccountPassword" IsHidden="yes" Value="$($PlainPassword)"/>
    <Parameter Id="ManagementServiceServerPort" Value="12045"/>
    <Parameter Id="ManagementServiceFirewallOption" Value="false"/>
    <Parameter Id="NavServiceClientServicesPort" Value="12046"/>
    <Parameter Id="WebServiceServerPort" Value="12047"/>
    <Parameter Id="WebServiceServerEnabled" Value="false"/>
    <Parameter Id="DataServiceServerPort" Value="12048"/>
...
"@
    ...
    [bool] installNav([xNavSetup]$obj) {
        if ((Test-Path $obj.SetupPath) -and (Test-Path $obj.ConfigFile)) {
            ...
            #Write-Verbose 'Start-Process -FilePath "C:\windows\system32\windowspowershell\v1.0\powershell.exe" -ArgumentList $arguments -Wait -PassThru'
            ...
            if ($setup.ExitCode -in @(0, 3010)) {
                ...
                "Nav successfully installed. Exit Code " + $setup.ExitCode | out-file "$($env:TEMP)\navsetup.log" -Append
    ...
    [void] patchNav([xNavSetup]$obj) {
        ...
        #Patchen
        "Patching RTC" | out-file "$($env:TEMP)\navsetup.log" -Append
        Copy-Item -Path "$($obj.PatchPath)\RTC\*" -Destination "C:\Program Files (x86)\Microsoft Dynamics NAV\100\RoleTailored Client" -Force -Recurse -PassThru
        "Patching NST" | out-file "$($env:TEMP)\navsetup.log" -Append
        Copy-Item -Path "$($obj.Patchpath)\NST\*" -Destination "C:\Program Files\Microsoft Dynamics NAV\100\Service" -Force -Recurse -PassThru
		
        if ($obj.InstanceName -ne "DynamicsNAV100") {
            "Upgrading Database" | out-file "$($env:TEMP)\navsetup.log" -Append
            Start-Process -FilePath 'C:\Program Files\Microsoft Dynamics NAV\100\Service\finsql.exe' -ArgumentList "Command=Upgradedatabase,", $("Database=" + $($obj.InstanceName) + ","), "ServerName=LOCALHOST," -ErrorAction SilentlyContinue -Wait -PassThru

            #Start Service
            "Starting Service $("MicrosoftDynamicsNavServer`$"+ $($obj.InstanceName))" | out-file "$($env:TEMP)\navsetup.log" -Append
            Set-Service -Name $("MicrosoftDynamicsNavServer`$"+ $($obj.InstanceName)) -StartupType Automatic -Status Running
        }

        "Done" | out-file "$($env:TEMP)\navsetup.log" -Append
    }
}
...
class xNavSetupOnCore {
    ...
    [void] CreateSuperUser ([string]$userName) {

        $serviceTierFolder = (Get-Item "C:\Program Files\Microsoft Dynamics NAV\100\Service").FullName
        Import-Module ([System.IO.Path]::Combine($serviceTierFolder, 'Microsoft.Dynamics.Nav.Management.psm1'))

        try {
            Write-Verbose "Lege $($userName) an"
            New-NavServeruser -ServerInstance $this.DatabaseName -WindowsAccount $userName
            New-NAVServerUserpermissionSet -permissionsetid SUPER -ServerInstance $this.DatabaseName -WindowsAccount $userName
        }     
        catch {

            Write-Verbose $("Fehler beim berechtigen des Users " + $userName + ": " + $_.Exception.Message)
        }
    }

    [void] PatchNav([string]$patchPath, [string]$instanceName) {
    
        Write-Verbose "Beende Dienste"
        Get-Service -Name "MicrosoftDynamicsNavServer`$$($instanceName)" | Stop-Service -Force -ea 0
        Get-Process -Name "*NAV*" | Stop-Process -Force -ea 0
        Get-Process -Name "*finsql*" | Stop-Process -Force -ea 0
        Get-Process -Name "*MMC*" | Stop-Process -Force -ea 0

        Write-Verbose "Patche"
        Copy-Item -Path  ([System.IO.Path]::Combine($patchPath, 'NST', '*')) -Destination "C:\Program Files\Microsoft Dynamics NAV\100\Service" -Force -Recurse -Exclude "CustomSettings.config"
      
        Write-Verbose "Starte Dienste"
        Get-Service -Name "MicrosoftDynamicsNavServer`$$($instanceName)" | Start-Service
    }
}
...
class xNavInstance {
    ...
    [void] Set() {
        if ($this.Ensure -eq [Ensure]::Present -and (Test-Path -Path 'HKLM:\SOFTWARE\Microsoft\Microsoft Dynamics NAV\100\Service')) {
            ...
            Import-Module $managementDllPath -ErrorVariable errorVariable -ErrorAction SilentlyContinue -Force
            ...
            if ($instance) {
                ...
                foreach ($user in $this.SuperUser) {
                    try {
                        New-NavServeruser -ServerInstance $this.InstanceName -WindowsAccount $user
                        New-NAVServerUserpermissionSet -permissionsetid SUPER -ServerInstance $this.InstanceName -WindowsAccount $user
                    }     
                    catch {
                        Write-Verbose $("Fehler beim berechtigen des Users " + $user + ": " + $_.Exception.Message)
                    }
                }           
            }
...
<Configuration>
	...
	<Parameter Id="WebClientDependencyBehavior" Value="install"/>
	<Parameter Id="NavHelpServerPath" Value="[WIX_SystemDrive]\Inetpub\wwwroot"/>
	<Parameter Id="NavHelpServerName" Value="s00-nsys-web"/>
